...
export function Feed() {
    ...
    async function loadFeed() {
      ...
      try {
        ...
        for (const userDoc of usersSnapshot.docs) {
          ...
          const socialDocRef = doc(db, "users", userId, "social", "links")
          const socialSnapshot = await getDoc(socialDocRef)

          let socialLinks: SocialLinksProps | undefined
          if (socialSnapshot.exists()) {
            const data = socialSnapshot.data()
            socialLinks = {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-screen">
        <Header />
        <p className="text-white text-lg font-medium">Carregando feed...</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col items-center min-h-screen pb-7 px-2">
      ...
      <main className="flex flex-col w-11/12 max-w-2xl gap-8">
        {users.map((user) => (
          <section
            key={user.uid}
